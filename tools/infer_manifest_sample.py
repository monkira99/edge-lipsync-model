...
from edge_lipsync.inference import run_manifest_sample_inference  # noqa: E402


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run inference for one manifest sample and write smoke-test artifacts."
    )
    ...
    parser.add_argument("--wenet-onnx", default="")
    parser.add_argument("--alpha-bin", default="")
    parser.add_argument("--output-mp4", nargs="?", const="output.mp4", default="")
    ...
    print(f"prediction={artifacts['prediction_path']}")
    print(f"grid={artifacts['grid_path']}")
    print(f"restored_frame={artifacts['restored_frame_path']}")
